...
1. **FIRST**: Check if `manage.py` exists in the project root.

   **If NO manage.py**: This is a new, uninitialized project. Tell the user:
   > "This looks like a new project from template. Run `/init` to initialize it with your project name and domain."

   Then STOP - don't proceed with the rest of startup.

2. Read the `.env` file to determine the environment:
   - `ENVIRONMENT=local` = Local development machine
   - `ENVIRONMENT=development` = Dev server
   - `ENVIRONMENT=production` = Production server

3. Read `claude/CODEBASE_STATE.md` - current technical state, recent work, and next planned tasks
4. Read `claude/CONTEXT_MEMORY.md` - working context and ongoing priorities
5. Read `claude/DEV_HANDOFF.md` if it exists - check for pending server-side actions
6. Read any files in `claude/specs/` if they exist (PRD, requirements, etc.)
7. List all files in `claude/session_progress/` and read the most recent 2-3 files
